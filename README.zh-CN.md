...
为了避免进入原生全屏时任务条闪烁一帧，钨极会监听全局鼠标左键按下和键盘按下事件，只识别标准窗口绿灯和精确的 `Control-Command-F`。普通输入不会被记录、写入日志、修改或发送到任何地方。监听默认开启；在设置中关闭「进入全屏时避免闪烁」后，相关监听会被完全停用。

macOS 在密码框等场景启用 Secure Input 时，不会把键盘事件交给全局 event tap，因此这段时间无法提前识别全屏快捷键；绿灯识别不受影响。
